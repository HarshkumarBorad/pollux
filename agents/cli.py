"""CLI for testing individual agents standalone.

Each subcommand maps to one specialist. The coordinator (Phase 4) routes to
the right specialist automatically; this CLI bypasses routing so you can
exercise each agent in isolation.
...
async def cmd_list(_args) -> int:
    ...
    for card in cards:
        domain_suffix = ""
        # Pull domain from the class — cards don't carry it directly.
        from agents import AGENT_REGISTRY
        cls = AGENT_REGISTRY.get(card.id)
        if cls and cls.domain:
            domain_suffix = f"  [domain={cls.domain.value}]"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run a single Pollux agent.")
    sub = parser.add_subparsers(dest="cmd", required=True)

    sub.add_parser("list", help="List all registered agents and capabilities.")

    p_hr = sub.add_parser("hr", help="Run the HR Specialist.")
    p_hr.add_argument("question")

    p_it = sub.add_parser("it", help="Run the IT/Tech Specialist.")
    ...
    p_ops.add_argument(
        "--attendees", default="", help="Comma-separated list of attendee names."
    )
